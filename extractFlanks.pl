...
#OTHER SYSTEM CALLS INCORPORATE MODULES FROM hhmmir (http://biodev.hgen.pitt.edu/kadriAPBC2009.html)
#
#############################################################################################################################
...
while (<$samFile>){
	...
	push @{$samin{$fastaref}},[$readname,$begin,$finish,$readstack,$start,$flag];
...
while (my $seqobj = $wholeseq->next_seq){
    ...
    unless(defined $samin{$seqid}){
        print "check sam matches fasta\n";
	next;	
        }
    ...
    for (@{$samin{$seqid}}){
        ...
        my ($readname, $begin, $finish,$readstack,$start,$flag) = @$_;
        warn join("\t",$readname,$begin,$finish,"$readstack...extracted"),"\n";

        open(my $outfile, ">>", "$fasta.extracts");
        print $outfile ">$seqid:$readname:$readstack:$begin-$finish:$start\n";

        if (($flag == 16) or ($flag == 272)){
            my $revcom = substr($seq,$begin,$finish-$begin+1);
            $revcom = reverse($revcom);
            $revcom =~ tr/AGCTagct/TCGAtcga/;
            print $outfile $revcom . "\n";
        }
